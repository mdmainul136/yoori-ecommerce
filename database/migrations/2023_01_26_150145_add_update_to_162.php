...
class AddUpdateTo162 extends Migration
{
    public function up()
    {
        ...
        if (
            !in_array(['package_update'], $permissions) && !in_array(['package_create'], $permissions) && !in_array(['package_destroy'], $permissions)
        ...
        //     $table->unsignedBigInteger('seller_country_id')->after('shop_tagline')->default(19);
        // });
        ...
        if (!Schema::hasColumn('sellers', 'seller_country_id')) {
            Schema::table('sellers', function (Blueprint $table) {
                $table->unsignedBigInteger('seller_country_id')->after('shop_tagline')->default(19);
            });
        }
        ...
        if (!Schema::hasColumn('carts', 'is_buy_now')) {
            Schema::table('carts', function (Blueprint $table) {
                $table->boolean('is_buy_now')->after('product_referral_code')->default(0);
            });
        }
    ...
    public function down()
    {
        if (Schema::hasColumn('sellers', 'seller_country_id')) {
            Schema::table('sellers', function (Blueprint $table) {
                $table->dropColumn('seller_country_id');
            });
        }
        ...
        // Optional: Remove permission if needed
        $permission = Permission::where('attribute', 'package')->first();
        if ($permission) {
            $permission->delete();
        }
