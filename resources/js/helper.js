export const mixing = {
    ...
    computed: {
        ...
        productSold(){
            return this.$store.getters.getProductSold;
        },


        SortFilter() {
            return [
                { label: this.lang.show_12, value: 12 },
                { label: this.lang.show_16, value: 16 },
                { label: this.lang.show_20, value: 20 },
                { label: this.lang.show_24, value: 24 },
            ];
        },
        ...
        SortByOptions() {
            return [
                ...
                { label: this.lang.top_rated, value: 'top_rated' },
                { label: this.lang.top_selling, value: 'top_selling' }
            ];
        },
    ...
    methods: {
        ...
        likeReply(id, comment_id) {
            ...
            axios
                ...
                .then((response) => {
                    ...
                    if (response.data.error) {
                        this.$toast.error(response.data.error, this.lang.success);
                    } else {
                        this.comment.comment_replies =
                            response.data.comment.comment_replies;
        ...
        formatCurrency(amount, symbol, decimals, options = {}) {
            const {
                symbolOnLeft = true,
                thousandsSeparator = ',',
                decimalSeparator = '.',
            ...
            let fixed = parseFloat(amount).toFixed(decimals);
        ...
        priceFormat(amount, only_amount) {
            ...
            const no_of_decimals = this.settings.no_of_decimals;
            const decimal_separator = this.settings.decimal_separator || '.';
            const thousands_separator = decimal_separator === ',' ? '.' : ',';
            const currency_symbol_format = this.settings.currency_symbol_format || 'amount_symbol';
            ...
            switch (currency_symbol_format) {
                case 'amount_symbol':
                    formatted_amount = this.formatCurrency(
                        amount,
                        symbol,
                        no_of_decimals,
                        { ...commonOptions, symbolOnLeft: false }
                    );
                    break;
                case 'symbol_amount':
                    formatted_amount = this.formatCurrency(
        ...
        //     decimal_separator = this.settings.decimal_separator
        //         ? this.settings.decimal_separator
        //         : ".";
        //     thousands_separator = decimal_separator == "," ? "." : ",";
        //     currency_symbol_format = this.settings.currency_symbol_format
        //         ? this.settings.currency_symbol_format
        //         : "amount_symbol";

        //     if (currency_symbol_format == "amount_symbol")
        //         formatted_amount = this.$options.filters.currency(
        //             amount,
        //             this.activeCurrency.symbol,
        //             no_of_decimals,
        //             {
        //                 symbolOnLeft: false,
        //                 thousandsSeparator: thousands_separator,
        ...
        productFetch(slug) {
            ...
            const modal = new bootstrap.Modal(document.getElementById('productPopup'));
            modal.show();
