    ...
    <tbody>
        ...
        @for ($i = 0; $i < session()->get('attributes'); $i++)
            <tr>
                ...
                <td width="18%">
                    <input type="number" lang="en" name="variant_price[{{ $i }}]"
                        value="{{ @old('variant_price')[$i] }}" min="0" step="any" class="form-control" required>
                    @if ($errors->has('variant_price.' . $i))
                        <div class="invalid-feedback">
                            <p>{{ $errors->first('variant_price.' . $i) }}</p>
                        </div>
                    @endif
                </td>
                ...
                <td width="18%">
                    <input type="text" name="variant_sku[{{ $i }}]"
                        value="{{ @old('variant_sku')[$i] }}" class="form-control" required>
                ...
                <td width="18%">
                    <input type="number" lang="en" name="variant_stock[{{ $i }}]"
                        value="{{ @old('variant_stock')[$i] }}" min="0" step="1" class="form-control" required>

                    @if ($errors->has('variant_stock.' . $i))
                        <div class="invalid-feedback">
                            <p>{{ $errors->first('variant_stock.' . $i) }}</p>
                        </div>
                    @endif
                </td>
                <td>
                    <div>
                        <div class="form-group">
                            <div class="input-group gallery-modal" id="btnSubmit" data-for="image" data-variant="1"
                                ...
                                <input type="hidden" name="variant_image[{{ $i }}]"
                                    value="{{ @old('variant_image')[$i] }}" class="image-selected">
