                    ...
                    <div class="card">
                        ...
                        <div class="card-body card-body-paddding">
                            ...
                            <form method="POST" action="{{ $route }}" enctype="multipart/form-data">
                                ...
                                <input type="hidden" class="form-control" name="sub_title" id="sub_title"
                                   value="sub title" />
                                ...
                                <div class="form-group">
                                    ...
                                    <div>
                                        ...
                                        @php
                                            $images = @$edit->image['images'] ?? null;
                                            $storage = $images['storage'] ?? null;
                                        @endphp

                                        @if($images)

                                            @php
                                                $isSvg = isset($images['original_image']) && Str::endsWith(strtolower($images['original_image']), '.svg');
                                            @endphp

                                            @if($isSvg)
                                                @if(is_file_exists($images['original_image'], $storage))
                                                    <img src="{{ get_media($images['original_image'], $storage) }}" alt="" id="img_image" class="img-thumbnail site-icon" />
                                                @else
                                                    <img src="{{ static_asset('images/default/default-image.svg') }}" alt="" id="img_image" class="img-thumbnail site-icon" />
                                                @endif
                                            @else
                                                @if(is_file_exists($images['image_72x72'] ?? null, $storage))
                                                    <img src="{{ get_media($images['image_72x72'], $storage) }}" alt="" id="img_image" class="img-thumbnail site-icon" />
                                                @else
                                                    <img src="{{ static_asset('images/default/default-image-72x72.png') }}" alt="" id="img_image" class="img-thumbnail site-icon" />
                                                @endif
                                            @endif
