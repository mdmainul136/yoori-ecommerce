            ...
            <table width="100%">
                ...
                <tr>
                    <td width="100%">
                        ...
                        <p>{{ __('Payment Type') }}
                            : {{ $order->payment_type == 'cash_on_delivery' ? __('Cash on Delivery') : ''}}</p>
...
<table class="items" width="100%" style="border-spacing:3px; font-size: 9pt; border-collapse: collapse;"
    ...
    @foreach ($order->orderDetails as $key => $orderDetail)
        <tr style="border-bottom: 1px solid #ccc;">
            <td>{{ $key+1 }}</td>
            <td>{{ !blank($orderDetail->product) ? $orderDetail->product->getTranslation('name') : 'N/A' }} @if($orderDetail->variation != null)
