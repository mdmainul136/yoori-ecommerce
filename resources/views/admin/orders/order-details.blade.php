                                ...
                                <div class="row">
                                    <div class="col-md-6 col-12 col-sm-6">
                                        <address>
                                            ...
                                            {{ arrayCheck('name',$order->billing_address) ? @$order->billing_address['name'] : '' }}
                                            <br>
                                            {{ arrayCheck('email',$order->billing_address) ? @$order->billing_address['email'] : '' }}
                                            <br>
                                            {{ arrayCheck('phone_no',$order->billing_address) ? @$order->billing_address['phone_no'] : '' }}
                                            <br>
                                    ...
                                    <div class="col-md-6 col-12 col-sm-6 text-md-right">
                                        <address>
                                            ...
                                            {{ arrayCheck('name',$order->shipping_address) ? @$order->shipping_address['name'] : '' }}
                                            <br>
                                            {{ arrayCheck('email',$order->shipping_address) ? @$order->shipping_address['email'] : '' }}
                                            <br>
                                            {{ arrayCheck('phone_no',$order->shipping_address) ? @$order->shipping_address['phone_no'] : '' }}
                                            <br>
                                ...
                                <table class="table table-striped table-hover table-md">
                                    ...
                                    @foreach ($order->orderDetails as $key => $orderDetail)
                                        @php
                                            ...
                                            $coupon_discount = $orderDetail->retail_price > 0 ? $orderDetail->coupon_discount['retail_discount'] : $orderDetail->coupon_discount['discount'];
                                            $tax = ($orderDetail->retail_tax > 0 ? $orderDetail->retail_tax : $orderDetail->tax) * $orderDetail->quantity;
                                            $shipping_cost = $orderDetail->shipping_cost['total_cost'];
                                            $discount = ($orderDetail->retail_discount > 0 ? $orderDetail->retail_discount : $orderDetail->discount) * $orderDetail->quantity;
                                            $sub_total = (($price * $orderDetail->quantity) + $tax + $shipping_cost) - $discount;
                                        @endphp
                                        <tr>
                                            <td>{{ $key+1 }}</td>
                                            <td>
                                            ...
                                            <td class="text-center">{{ get_price($price,user_curr()) }}</td>
                                            <td class="text-center">{{ $orderDetail->quantity }}</td>
                                            <td class="text-center txt_nowrap">
                                                {{ get_price($price,user_curr()) }}
                                                X {{ $orderDetail->quantity }}
                                                = {{ get_price($price * $orderDetail->quantity,user_curr()) }}
                                            </td>
                                            @if(($order->tax_method && $order->tax_method['vat_tax_type'] == 'product_base') || (!$order->tax_method || !$order->tax_method['vat_tax_type']))
                                                @if($orderDetail->tax > 0)
                                                    <td class="text-center txt_nowrap">
                                                        {{ get_price($orderDetail->retail_tax > 0 ? $orderDetail->retail_tax : $orderDetail->tax, user_curr()) }}
                                                        X {{ $orderDetail->quantity }}
                                            ...
                                            @if($discount > 0)
                                                <td class="text-center txt_nowrap">
                                                    {{ get_price($orderDetail->retail_discount > 0 ? $orderDetail->retail_discount : $orderDetail->discount,user_curr()) }}
                                                    X {{ $orderDetail->quantity }}
                    ...
                    <div class="modal-body modal-padding-bottom modal-body-overflow-unset">
                        ...
                        <div class="form-group align-items-center">
                            ...
                            <input type="text" id="total_payable"
                                   value="{{ get_price($order->retail_total_payable > 0 ? $order->retail_total_payable : $order->total_payable,user_curr()) }}"
                                   class="form-control" readonly="" disabled="">
                        </div>
                        <div class="form-group">
