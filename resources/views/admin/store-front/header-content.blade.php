                                ...
                                <div class="card-body col-md-10 middle">
                                    <form method="post" action="{{route('update')}}" enctype="multipart/form-data">
                                        ...
                                        <div class="form-group seo-image-positoin">
                                            <label for="dark_logo">{{ __('Logo') }} {{__('(138X52)')}}</label>
                                            <div class="form-group">
                                            ...
                                            <div>
                                                ...
                                                @php
                                                    $darkLogo = settingHelper('dark_logo');
                                                    $imagePath = '';

                                                    if (!empty($darkLogo) && is_array($darkLogo)) {
                                                        if (!empty($darkLogo['image_138x52']) && is_file_exists($darkLogo['image_138x52'])) {
                                                            $imagePath = static_asset($darkLogo['image_138x52']);
                                                        } elseif (!empty($darkLogo['original_image']) && is_file_exists($darkLogo['original_image'])) {
                                                            $imagePath = static_asset($darkLogo['original_image']);
                                                        }
                                                    }

                                                    $imagePath = $imagePath ?: static_asset('images/default/default-image-72x72.png');
                                                @endphp

                                                <img src="{{ $imagePath }}" alt="Dark Logo" id="img_dark_logo" class="img-thumbnail site-icon" />
