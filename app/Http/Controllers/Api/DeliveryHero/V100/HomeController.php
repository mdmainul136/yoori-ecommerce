...
class HomeController extends Controller
{
    ...
    public function homePageData(Request $request)
    {
        try {
            ...
            if (!$user || !$user->deliveryHero) {
                return $this->responseWithError(__('delivery_hero_not_found'), [], 404);
            }
            ...
            $data['total_collection']     = get_price($this->deliveryHero->totalCollection($delivery_hero_id));
            $data['today_collection']     = get_price($this->deliveryHero->todayCollection($delivery_hero_id));
