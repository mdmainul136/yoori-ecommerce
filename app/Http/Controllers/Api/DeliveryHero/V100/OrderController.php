...
class OrderController extends Controller
{
    ...
    public function orders(Request $request, $slug)
    {
        try {
            ...
            if ($slug == 'pending'):
                $data = OrderResource::collection($user->deliveryHero->orders->whereNotIn('delivery_status', ['delivered', 'canceled'])->orderBy('id', 'DESC')
                    ->get());
            elseif ($slug == 'canceled'):
                $data = OrderResource::collection($user->deliveryHero->orders->where('delivery_status', 'canceled')->orderBy('id', 'DESC')
                    ->get());
