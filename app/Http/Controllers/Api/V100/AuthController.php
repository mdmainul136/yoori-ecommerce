            ...
            if($user):
                ...
                if ($url)
                {
                  $tempPath = tempnam(sys_get_temp_dir(), 'image');
						...
						$imageExtension = pathinfo(parse_url($request->image, PHP_URL_PATH), PATHINFO_EXTENSION) ?: 'jpg';  // Default to jpg if no extension

						$requestImage = new \Illuminate\Http\UploadedFile(
							$tempPath,                            // Temporary file path
							'downloaded_image.' . $imageExtension, // The original filename or custom name
							null,                                 // MIME type (optional)
							null,                                 // Size (optional)
							true                                  // Mark as a test file
						);

						$images = $this->saveImage($requestImage, '_staff_');
                }
