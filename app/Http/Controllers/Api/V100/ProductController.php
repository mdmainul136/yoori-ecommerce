        ...
        try {
            ...
            if ($product->images) {
                foreach ($product->images as $image) {
                    if (
                        $image &&
                        (arrayCheck(['image_190x230'], @$image['storage']) || is_file_exists(@$image['image_190x230'], @$image['storage']))
                    ) {
                        $images[] = get_media(@$image['image_190x230'], @$image['storage']);
                        $image_no++;
                    }
                }
            }
            ...
            if ($product->has_variant) {
                foreach ($stock as $key => $item) {
                    ...
                    if (
                        $item->image &&
                        (arrayCheck([$size], @$item->image['storage']) || is_file_exists(@$item->image[$size], @$item->image['storage']))
                    ) {
