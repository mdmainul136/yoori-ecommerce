...
class ProductController extends Controller
{
    ...
    // frontend product details ... logic
    public function productDetails(Request $request, $slug, ColorInterface $color, AttributeInterface $attribute): \Illuminate\Http\JsonResponse
    {
        ...
        try {
            ...
            $product->product_stock = [
                ...
                'price' => supplierProductPriceCalculator($first_stock->price, $product),
            ...
            $product->price = supplierProductPriceCalculator($product->price, $product);
            ...
            $product['color_wise_variant'] = $product->color_wise_variant;
            ...
            // return $product->color_wise_variant;
    ...
    // get breadcrumb slug
    // public function breadcrumbSlug($slug){
    //     $category = \App\Models\Category::with('categoryLanguage')->where('slug', $slug)->first();
    ...
    //     return response()->json(array_reverse($slugs));
    // }
    ...
    public function breadcrumbSlug($slug)
    {
        $category = \App\Models\Category::with(['categoryLanguage', 'parent.categoryLanguage'])
            ->where('slug', $slug)
            ->first();
        ...
        while ($category && $category->parent) {
            ...
            $breadcrumbs[] = [
                'slug'  => $parent->slug,
                'title' => optional(
                    $parent->categoryLanguage->where('lang', languageCheck())->first()
                )->title ?? $parent->title
            ];
        ...
        return response()->json(array_reverse($breadcrumbs));
    }




    public function breadcrumbTitle($slug, $routeName){

        // return $routeName;
        // categoryLanguage

        if($routeName == 'product.by.brand'){
            $brand = \App\Models\Brand::with('brandLanguages')->where('slug', $slug)->first();
            $title = $brand->brandLanguages->where('lang', languageCheck())->first();
            return response()->json($title->title);
        }

        else{
            $category = \App\Models\Category::with('categoryLanguage')->where('slug', $slug)->first();
            $title = $category->categoryLanguage->where('lang', languageCheck())->first();
            return response()->json($title->title);
        }
    ...
    public function variants(Request $request, AttributeInterface $attribute, ColorInterface $color)
    {
        // return $request->all();
        ...
        try {
            ...
            $variants = $attribute->actualStocks($request->color_id, $request->product_id);
    ...
    public function findVariants(Request $request, AttributeInterface $attribute)
    {
        // return $request->all();
        ...
        try {
            ...
            if ($request->color_id) {
                ...
                // return "okay";
                $variant_ids = array_values(array_filter($request->variant_ids))[0];
                $stock = $attribute->findStock($variant_ids, $request->product_id);
            }
    ...
    public function filterProducts(Request $request, ProductInterface $product, CategoryInterface $category, BrandInterface $brand, SellerProfileInterface $profile): \Illuminate\Http\JsonResponse
    {
        try {
            ...
            if ($request->route == 'product.by.category' || $request->route == 'product.by.gadget') {
                ...
                // src="http://localhost/Yoori-Laravel/public/images/default/default-image_970x270.png"

                $page['child_categories'] = $child_categories;

                if ($page->banner && is_file_exists(getArrayValue('image_970x270', $page->banner), $page->banner['storage'])) {
                    $page->image_835x200 = @getFileLink('970x270', $page->banner);
                } else {
                    $page->image_835x200 = @getFileLink('970x270', settingHelper('category_default_banner')['images']);
                }
