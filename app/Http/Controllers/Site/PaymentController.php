...
class PaymentController extends Controller
{
    ...
    public function stripeRedirect(Request $request)
    {
        ...
        // return settingHelper('stripe_secret');
        try {
            ...
            // return $response = httpRequest('https://api.stripe.com/v1/checkout/sessions', $session_data, $headers, true);
    ...
    */
    ...
    // This function handles Razorpay payment order creation.
    // It takes the request data, finds the related order and amount,
    // creates a Razorpay order, stores important info in session,
    // and returns the data needed for the frontend to complete the payment.
    public function razorpayRedirect(Request $request)
    {
        try {
            // Initialize Razorpay API with keys from settings
            $api = new Api(settingHelper('razorpay_key'), settingHelper('razorpay_secret'));

            // Find the related orders and active currency
            $orders = $this->findOrders($request->all());
            $active_currency = $this->activeCurrencyCheck();
            $amount = $this->findAmount($request->all(), $orders, $active_currency);

            // If no orders found, return an error
            if (empty($orders)) {
                return response()->json(['error' => __('Oops.....Something Went Wrong')], 400);
            }

            // Convert amount to paisa (Razorpay accepts amount in smallest currency unit)
            $amount_in_paisa = (int) round($amount * 100);

            // Create a new order in Razorpay
            $razorpayOrder = $api->order->create([
                'receipt' => uniqid(), // Generate unique receipt ID
                'amount' => $amount_in_paisa,
                'currency' => $active_currency->code ?? 'INR',
                'payment_capture' => 1 // Auto-capture payment
            ]);

            // Save order ID and transaction ID in session for later use
            session()->put('razorpay_order_id', $razorpayOrder['id']);
            session()->put('razorpay_order_trx_id', $request['trx_id']);

            // Return all required Razorpay details to frontend
            return response()->json([
                'order_id' => $razorpayOrder['id'],
                'razorpay_key' => settingHelper('razorpay_key'),
                'amount' => $amount_in_paisa,
                'currency' => $active_currency->code ?? 'INR',
                'name' => settingHelper('admin_panel_title', 'en'),
    ...
    // This function verifies Razorpay payment using the signature returned from Razorpay.
    // If the verification is successful, it completes the order and clears session data.

    public function razorpayPaymentVerify(Request $request, OfflineMethodInterface $offlineMethod)
    {
        try {
            // Initialize Razorpay API with your secret keys
            $api = new Api(settingHelper('razorpay_key'), settingHelper('razorpay_secret'));

            // Prepare the data Razorpay sends back for verification
            $attributes = [
            ...
            // Verify Razorpay's signature to ensure the payment is valid
            $api->utility->verifyPaymentSignature($attributes);

            // If signature is valid, prepare data for completing the order
            $data = [
                "id" => "", // Optional, leave blank or pass file ID if needed
                "file" => "", // Optional, used for file-based orders
                'payment_type' => 'razorpay', // Payment method used
                'trx_id' => session()->get('razorpay_order_trx_id'), // Stored during initial payment request
                "is_buy_now" => 0, // Indicates whether it's an instant buy
                'code' => $request->code ?? null, // Optional coupon or referral code
            ];

            // Complete the order process using your order service
            $this->order->completeOrder($data, authUser(), $offlineMethod);

            // Clear Razorpay-specific session data
            session()->forget('razorpay_order_id');
            session()->forget('razorpay_order_trx_id');
            ...
            // If something goes wrong (e.g. signature mismatch), return error response
    ...
    public function razorpayWalletRedirect(Request $request)
    {
        // return
        try {
            ...
            if ($amount_in_paisa < 100) {
                return response()->json(['error' => 'Minimum recharge amount is ₹1'], 400);
            }
            ...
            // Save order ID and transaction ID in session for later use
            session()->put('razorpay_wallet_amount', $amount);
    ...
    public function razorpayWalletPaymentVerify(Request $request, WalletInterface $wallet)
    {
        try {
            // Initialize Razorpay API with your secret keys
            $api = new Api(settingHelper('razorpay_key'), settingHelper('razorpay_secret'));
            ...
            $data = [
                "id" => "", // Optional, leave blank or pass file ID if needed
                "file" => "", // Optional, used for file-based orders
                'payment_type' => 'razor_pay', // Payment method used
                'status' => 'success',
                'amount' => session()->get('razorpay_wallet_amount')
            ];


            $wallet->walletRecharge($data);


            // Clear Razorpay-specific session data
            session()->forget('razorpay_wallet_amount');
