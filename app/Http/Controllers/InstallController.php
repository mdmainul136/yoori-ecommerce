...
class InstallController extends Controller
{
    ...
    public function createRelease(Request $request)
    {
        ...
        try {
            ...
            $gitDiffCommand = "git diff --name-only $latest_commit $old_commit";// Execute the command
            $changedFiles = shell_exec($gitDiffCommand);
            file_put_contents(base_path('release_creator.txt'), $changedFiles);
            $file = base_path('release_creator.txt');//now i'll take each line of the file and put it into an array
            $lines = file($file);
            $data = [];
    ...
    public function final()
    {
        try {
            ...
            $config = [
                "app_version" => "120",
                "app_version_code" => "1.2.0",
                "web_version" => "201",
                "web_version_code" => "2.0.1",
                "removed_directories" => [
