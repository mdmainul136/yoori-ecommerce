...
class CartRepository implements CartInterface
{
    ...
    public function addToCart($request, $product,$user)
    {
        ...
        $retail_price = supplierProductPriceCalculator($product_stock->price, $product);
        ...
        if ($product->is_wholesale):
            ...
            if (!blank($wholesale_price)):
                ...
                $retail_price = supplierProductPriceCalculator($wholesale_price->price, $product);
            endif;
        else:
            ...
            if (special_discount_applicable($product)):
                ...
                elseif ($product->special_discount_type == 'percentage'):
                    ...
                    $retail_discount   = ($retail_price * $product->special_discount) / 100;
                endif;
            endif;
        endif;
        ...
        if (settingHelper('vat_and_tax_type') == 'product_base'):
            foreach ($product->vatTaxes($product) as $product_tax) :
                ...
                $retail_tax += ($retail_price * $product_tax->percentage) / 100;
            endforeach;
        endif;
        ...
        $parse_cart = $this->userCarts($request, $carts, $variant, $product, $price, $discount, $tax, $shipping_cost,$user,$trx_id,$retail_price,$retail_discount,$retail_tax);
    ...
    protected function userCarts($request, $carts, $variant, $product, $price, $discount, $tax, $shipping_cost,$user,$trx_id,$retail_price = 0,$retail_discount = 0,$retail_tax = 0)
    {
        if (count($carts) > 0):
            $cart_exist = false;
            foreach ($carts as $cart_item):
                ...
                if (($variant != null && $cart_item['variant'] == $variant) || $variant == null):
                    ...
                    if ($cart_product->is_wholesale):
                        ...
                        if (!blank($wholesale_price)):
                            $price = $wholesale_price->price;
                            $retail_price = supplierProductPriceCalculator($wholesale_price->price, $cart_product);
                        endif;
                    endif;

                    $cart_item->price = $price;
                    if (addon_is_activated('ramdhani')) {
                        $cart_item->retail_price = $retail_price;
                    }
            ...
            if (!$cart_exist) :
                ...
                if (addon_is_activated('ramdhani')) {
                    $cart_item->retail_price = $retail_price;
                    $cart_item->retail_discount = $retail_discount;
                    $cart_item->retail_tax = $retail_tax;
                }
    ...
    public function updateCart($request)
    {
        ...
        $cart_item->price = $price;
        if (addon_is_activated('ramdhani')) {
            $cart_item->retail_price = supplierProductPriceCalculator($price, $product);
        }
        $cart_item->shipping_cost = $shipping_cost;
        $cart_item->save();
        ...
        if (count($carts) > 0) {
            ...
            if (settingHelper('vat_and_tax_type') == 'order_base') {
                foreach ($seller_carts as $key => $seller_cart) {
                    $sub_total = $retail_sub_total = 0;
                    $tax_amount[$key] = $retail_tax_amount[$key] = 0;
                    foreach ($seller_cart as $cart) {
                        ...
                        {
                            $sub_total += $cart->price * $cart->quantity;
                            $retail_sub_total += $cart->retail_price * $cart->quantity;
                        }
                        else{
                            $sub_total += (($cart->price * $cart->quantity) + ($shipping_cost * $cart->quantity)) - (($cart->discount * $cart->quantity) + $cart->coupon_discount);
                            $retail_sub_total += (($cart->retail_price * $cart->quantity) + ($shipping_cost * $cart->quantity)) - (($cart->retail_discount * $cart->quantity) + $cart->coupon_discount);
                        }
                    }
                    ...
                    if (settingHelper('order_wise_tax_percentage'))
                    {
                        ...
                        $retail_tax_amount[$key] += ($retail_sub_total * settingHelper('order_wise_tax_percentage')) / 100;
                    }
                }
            }
            ...
            foreach ($seller_carts as $key => $seller_cart) {
                $group_carts[$key] = [
                    ...
                    'tax'           => arrayCheck($key, $tax_amount) ? $tax_amount[$key] : 0,
                    'retail_tax'    => arrayCheck($key, $retail_tax_amount) ? $retail_tax_amount[$key] : 0,
                ];
                $i++;
            }
