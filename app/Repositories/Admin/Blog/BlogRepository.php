...
class BlogRepository implements BlogInterface
{
    ...
    public function store($request)
    {
        ...
        if ($request->image):
            $blog->image            = $this->getImageWithRecommendedSize($request->image,385,260);
        ...
        if ($request->banner != ''):
            $blog->banner            = $this->getImageWithRecommendedSize($request->banner,853,446);
...
//        }
    ...
    {
        return Blog::with('currentLanguage','user:id,first_name,last_name')->where('status','published')->take(4)->latest()->select('id','image','image_id','slug', 'user_id')->get();
    }

    public function blogs($data)
    {
        $blogs = Blog::with('user:id,first_name,last_name');
