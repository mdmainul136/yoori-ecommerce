...
class ServiceRepository implements ServiceInterface
{
    ...
    public function store($request)
    {
        ...
        if ($request->image != ''):
            ...
            $service->image = $this->saveSvg($request->image, 'service_image');

            // if ($extension === 'svg') {
            //     // SVG ফাইল হলে
            // } else {
            //     // অন্য ফাইল হলে (jpg, png, webp, etc)
            //     $service->image = $this->saveImage($request->image, 'service_image');
            // }
    ...
    public function update($request, $id)
    {
        ...
        if ($request->image):
            $this->deleteImage(@$service->image['images']);
            $service->image = $this->saveSvg($request->image, 'service_image');
        endif;
        $service->position = $request->position;
        $service->save();
    ...
    {
        return Service::with('currentLanguage')->orderBy('position')->take(6)->get();
    }
}
