...
class ProductRepository implements ProductInterface
{
    ...
    public function paginate($request, $status, $limit, $product_for = null)
    {
        $products = Product::with('createdBy', 'productLanguages','user','stock')
            ...
            ->when($request->sq, function ($q) use ($request) {
    ...
    public function store($request)
    {
        ...
        if ($request->thumbnail):
            $files     = $this->getImageWithRecommendedSize($request->thumbnail,210,205);
            if ($files):
...
//            $product->created_by                = Sentinel::getUser()->id;
    ...
    {
        $with = ['userWishlist','brand'];
        return Product::with($with)->withAvg('reviews','rating')->withCount('reviews', 'orders')
            ->selectRaw('id,price,brand_id,special_discount,
                    minimum_order_quantity,current_stock,special_discount_type,
                    special_discount_start,special_discount_end,rating,thumbnail,
                    slug,reward,todays_deal,has_variant')
            ->ProductPublished()
            ->UserCheck()->IsWholesale()->IsStockOut()->latest()->take(15)->get();
    }
    ...
    {
        return Product::with('userWishlist', 'brand')
            ->withAvg('reviews','rating')
            ->withCount('reviews', 'orders')
            ->UserCheck()->IsWholesale()->IsStockOut()
            ->selectRaw('id,price,brand_id,special_discount,minimum_order_quantity,current_stock,special_discount_type,special_discount_start,special_discount_end,rating,total_sale,thumbnail,slug,reward,current_stock')
            ->ProductPublished()
            ->whereHas('orders')
    ...
    public function offerEndingSoon($limit = null)
    {
        ...
        return Product::with($with)->withAvg('reviews','rating')->withCount('reviews','orders')->UserCheck()->IsWholesale()->IsStockOut()->where('is_wholesale',0)
            ->selectRaw('id,user_id,brand_id,price,special_discount,is_wholesale,special_discount_type,minimum_order_quantity,current_stock,special_discount_start,special_discount_end,rating,thumbnail,slug,reward')
    ...
    {
        return Product::with('userWishlist','brand')->withAvg('reviews','rating')->withCount('reviews','orders')->where('special_discount', '>', 0)->where('special_discount_end', '!=', null)->ProductPublished()
    ...
    public function productByBestSelling($limit = null)
    {
        ...
        return Product::with('userWishlist', 'brand')->withAvg('reviews','rating')->withCount('reviews','orders')
            ->selectRaw('id,brand_id,price,special_discount,special_discount_type,special_discount_start,special_discount_end,rating,total_sale,thumbnail,slug,reward,current_stock')
    ...
    public function productByGadget($slug)
    {
        ...
        return Product::with('brand')->withAvg('reviews','rating')->withCount('reviews','orders')->where('category_id', $category->id)->ProductPublished()->UserCheck()->IsWholesale()->IsStockOut()->latest()->paginate(16);
    }
    ...
    {
        return Product::with( 'brand')->withAvg('reviews','rating')->withCount('reviews', 'orders')->ProductPublished()->UserCheck()->IsWholesale()->IsStockOut()
    ...
    public function productByCampaign($campaign_id, $paginate = null): \Illuminate\Contracts\Pagination\LengthAwarePaginator
    {
        ...
        return Product::withAvg('reviews','rating')->join('campaign_products', 'products.id', 'campaign_products.product_id')
            ...
            ->select('products.id', 'campaigns.id as campaign_id', 'campaigns.slug as campaign_slug', 'current_stock', 'products.price', 'special_discount', 'special_discount_type', 'rating',
                'text_color', 'start_date', 'end_date', 'products.special_discount_start', 'products.special_discount', 'products.special_discount_end', 'products.thumbnail', 'products.slug', 'reward','has_variant', 'products.minimum_order_quantity', 'products.is_catalog', 'products.is_classified')
    ...
    public function filterProduct($data): \Illuminate\Contracts\Pagination\LengthAwarePaginator
    {
        ...
        if (array_key_exists('key', $data) && !empty($data['key'])) {
            ...
            // new code price filtering
            if (array_key_exists('price', $data)) {

                if (is_string($data['price'])) {
                    $price = json_decode($data['price']);
                } elseif (is_array($data['price'])) {
                    $price = $data['price'];
                }
        ...
        return  $products->with('userWishlist', 'brand')->withAvg('reviews','rating')->withCount('reviews','orders')
            ->when($data['route'] == 'product.by.selling', function($q){
                $q->orderByDesc('total_sale');
            })
            ->selectRaw('id,category_id,brand_id,price,special_discount,special_discount_type,special_discount_start,special_discount_end,rating,slug,thumbnail,reward,current_stock,has_variant,minimum_order_quantity,todays_deal,total_sale')
            ->ProductPublished()
    ...
    {
        return Product::with('userWishlist')->withAvg('reviews','rating')->withCount('reviews', 'orders')->where('category_id', $id)->where('id', '!=', $product_id)
    ...
    public function bestSellingProduct($category_id)
    {
        ...
        return Product::with($with)->withAvg('reviews','rating')->withCount('reviews','orders')->where('category_id',$category_id)->ProductPublished()
    ...
    {
        return Product::with('userWishlist', 'brand')->withAvg('reviews','rating')->withCount('reviews', 'orders')->whereIn('id',$ids)->ProductPublished()->where('is_featured',1)
            ->UserCheck()->IsWholesale()->IsStockOut()
    ...
    {
        return Product::with('userWishlist')->withAvg('reviews','rating')->withCount('reviews','orders')->where('user_id',$user_id)->ProductPublished()->latest()
            ->selectRaw('id,brand_id,category_id,status,price,special_discount,special_discount_type,special_discount_start,special_discount_end,rating,slug,thumbnail,minimum_order_quantity,has_variant,reward,current_stock')
    ...
    {
        return Product::with('userWishlist')->withAvg('reviews','rating')->withCount('reviews','orders')->where('user_id',$user_id)->ProductPublished()->orderBy('total_sale','desc')
            ->selectRaw('id,brand_id,category_id,status,price,special_discount,special_discount_type,special_discount_start,special_discount_end,rating,slug,thumbnail,minimum_order_quantity,has_variant,reward,current_stock')
    ...
    {
        return Product::with('userWishlist')->withAvg('reviews','rating')->withCount('reviews','orders')->where('user_id',$user_id)->orderBy('total_sale','desc')->where('is_wholesale',0)
            ->where('special_discount', '>', 0)->where('special_discount_end', '!=', null)->ProductPublished()
            ->selectRaw('id,brand_id,category_id,status,price,special_discount,special_discount_type,special_discount_start,special_discount_end,rating,slug,thumbnail,minimum_order_quantity,has_variant,reward,current_stock')
            ->UserCheck()->IsWholesale()->IsStockOut()->take(12)->get();
    }
    ...
    {
        return Product::with('userWishlist')->withAvg('reviews','rating')->withCount('reviews','orders')->where('user_id', $user_id)->ProductPublished()
            ->orderBy('id', 'desc')
            ->selectRaw('id,brand_id,category_id,status,price,special_discount,special_discount_type,special_discount_start,special_discount_end,rating,slug,thumbnail,minimum_order_quantity,has_variant,reward,current_stock')
    ...
    {
        return Product::withAvg('reviews','rating')->withCount('reviews','orders')->ProductPublished()
            ->selectRaw('id,brand_id,category_id,status,price,special_discount,special_discount_type,special_discount_start,special_discount_end,rating,slug,thumbnail,minimum_order_quantity,has_variant,reward,current_stock')
    ...
    public function takeCampaignProducts($take, $id, $skip)
    {
        return Product::with('brand')
            ->addSelect([
                ...
                'start_date',
                'end_date',
                'products.special_discount_start',
                'products.special_discount',
                'products.special_discount_end',
                'products.thumbnail',
                'products.slug',
                'reward',
                'minimum_order_quantity',
                'has_variant',
                'orders_count' => OrderDetail::select(DB::raw('count(*)'))
                    ->whereColumn('product_id', 'products.id')
            ])
            ->withAvg('reviews','rating')->withCount('reviews')->join('campaign_products', 'products.id', 'campaign_products.product_id')
            ->join('campaigns', 'campaign_products.campaign_id', 'campaigns.id')
            ->join('users', 'products.user_id', 'users.id')->where('users.status', 1)
            ...
            // ->select('products.id', 'products.brand_id', 'campaigns.id as campaign_id','campaigns.slug as campaign_slug','current_stock', 'products.price', 'special_discount', 'special_discount_type', 'rating',
    ...
    {
        return Product::withCount('orders')->where('user_id',1)->where('id', '!=',$id)->ProductPublished()->UserCheck()->IsWholesale()->IsStockOut()->latest()->limit(4)->get();
    }
