...
class AttributeRepository implements AttributeInterface
{
    ...
    public function actualStocks($colorId, $product_id)
    {
        $stock = ProductStock::where('product_id', $product_id)
            ->get()
            ->filter(function ($stock) use ($colorId) {
                $firstPart = explode('-', $stock->variant_ids)[0] ?? null;
                return $firstPart == $colorId;
            })
            ->map(function ($stock) {
                return (int) explode('-', $stock->variant_ids)[1] ?? null; // second part
            })
            ->filter() // null বাদ দেয়
            ->values(); // index reset করে

        return AttributeValues::whereIn('id',$stock)->get();
