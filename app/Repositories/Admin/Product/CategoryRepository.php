...
//                });
    ...
    public function store($request)
    {
            ...
            if ($request->logo != ''):
                $category->logo        = $this->getImageArrayRecommendedSize($request->logo,[140,130,110],[190,95,110]);
            ...
            if ($request->banner != ''):
                ...
                $category->banner        = $this->getImageWithRecommendedSize($request->banner,970,270,true);
...
//            $category->is_digital        = $request->is_digital;
    ...
    public function subcategoryPage($slug)
    {
        ...
        $subcategories = Category::with(['categories' => function ($query) {
                $query->where('status', 1);
            }])
            ->where('parent_id', $category->id)
            ->where('status', 1)
            ->select('id', 'parent_id', 'logo', 'slug', 'banner')
            ->latest()
            ->get();
    ...
    // category with child Category
    public function categoryWithChild($ids,$take= null){
        ...
        return $take ? $categories->take($take) : $categories;
    }
