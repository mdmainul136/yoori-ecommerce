...
class BrandRepository implements BrandInterface
{
    ...
    public function store($request)
    {
            ...
            if ($request->logo != ''):
                $brand->logo        = $this->getImageWithRecommendedSize($request->logo, 150, 100);
                $brand->logo_id     = $request->logo;
            else:
                $brand->logo        = [];
            endif;
            if ($request->banner != ''):
                $brand->banner        = $this->getImageWithRecommendedSize($request->banner,970, 270,true);
    ...
    public function getActiveBrands($data = [])
    {
        return Brand::when(arrayCheck('q', $data), function ($query) use ($data){
            $query->whereHas('brandLanguages', function ($q) {
                $q->where('title', 'like', '%' . request()->q . '%');
            });
        })->when(arrayCheck('limit', $data), function ($query) use ($data) {
