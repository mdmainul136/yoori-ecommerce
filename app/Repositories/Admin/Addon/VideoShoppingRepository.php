...
class VideoShoppingRepository implements VideoShoppingInterface
{
    ...
    public function all()
    {
        return VideoShopping::with('user:id,first_name,last_name,user_type')->latest();
    }
    ...
    public function shopBySlug($slug)
    {
        ...
        if ($video)
        {
            $products = ProductResource::collection(Product::whereIn('id',$video->product_ids)->withCount('reviews', 'orders')->UserCheck()->IsWholesale()->IsStockOut()
            ...
            if ($video->enable_related_product == 1 && count($products) > 0)
            {
                ...
                $related_products = ProductResource::collection(Product::withCount('reviews', 'orders')->UserCheck()->IsWholesale()->IsStockOut()
                    ->selectRaw('id,price,special_discount,minimum_order_quantity,current_stock,special_discount_type,special_discount_start,special_discount_end,rating,total_sale,thumbnail,slug,reward,current_stock')
    ...
    public function shopBySlugApi($slug)
    {
        ...
        $video = VideoShopping::with('user:id,first_name,last_name,user_type')->where('slug',$slug)->active()->first();
