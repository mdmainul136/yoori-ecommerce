    ...
    public function confirmOrder($checkout, $carts, $data,$user)
    {
        ...
        if (count($carts) > 0)
        {
            foreach ($checkout as $key => $cart_group) {
                $sub_total = $retail_sub_total = $total_discount = $retail_total_discount = $total_tax = $retail_total_tax = $shipping_cost = 0;
                if (array_key_exists('seller_id',$cart_group))
                {
                    foreach ($carts->whereIn('seller_id', $cart_group['seller_id']) as $item) {
                        $sub_total += $item->price * $item->quantity;
                        $retail_sub_total += max($item->retail_price, 0) * $item->quantity;
                        $total_discount += $item->discount * $item->quantity;
                        $retail_total_discount += max($item->retail_discount, 0) * $item->quantity;
                        $total_tax += $item->tax * $item->quantity;
                        $retail_total_tax += max($item->retail_tax, 0) * $item->quantity;
                ...
                if (settingHelper('vat_type') == 'after_tax' && settingHelper('vat_and_tax_type') == 'order_base')
                {
                    ...
                    $retail_total_amount = ($retail_sub_total) - ($retail_total_discount + $coupon_discount);
                    $total_payable = $total_amount + $total_tax + $shipping_cost;
                    $retail_total_payable = $retail_total_amount + $retail_total_tax + $shipping_cost;
                }
                else{
                    $total_amount = ($sub_total + $total_tax) - ($total_discount + $coupon_discount);
                    $retail_total_amount = ($retail_sub_total + $retail_total_tax) - ($retail_total_discount + $coupon_discount);
                    $total_payable = $total_amount + $shipping_cost;
                    $retail_total_payable = $retail_total_amount + $shipping_cost;
                }
                $tax = [
                ...
                foreach ($carts->whereIn('seller_id', $cart_group['seller_id']) as $item) {
                    ...
                    if ($item->coupon_discount > 0) {
                        $coupon                 = [
                            ...
                            'retail_discount'   => $item->coupon_discount,
                        ];
                    }
