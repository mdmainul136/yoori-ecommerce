...
class CampaignRepository implements CampaignInterface
{
    ...
    public function store($request)
    {
            ...
            if ($request->thumbnail != ''):
                $campaign->thumbnail            = $this->getImageWithRecommendedSize($request->thumbnail,750,260,true);
    ...
    {
        return Campaign::whereIn('id',$ids)->select('id','banner','slug','thumbnail','start_date','end_date')->where('start_date','<=',now())->where('end_date','>=',now())->where('status',1)->latest()->take(5)->get();
    }
