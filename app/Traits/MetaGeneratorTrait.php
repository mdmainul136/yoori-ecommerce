...
trait MetaGeneratorTrait {
    public function generateMeta($product,$blog,$category,$brand,$seller): array
    {
        ...
        $lang = languageCheck();
        $system_og_image = settingHelper('footer_logo') != [] && @is_file_exists(settingHelper('footer_logo')['image_89x33']) ? get_media(settingHelper('footer_logo')['image_89x33'], settingHelper('footer_logo')['storage']) : static_asset('images/default/logo-89x33.png');

        if (settingHelper('og_image') && arrayCheck("image_1200x630", settingHelper('og_image')) && is_file_exists(settingHelper('og_image')['image_1200x630']))
        {
            $system_og_image = getFileLink('1200x630',settingHelper('og_image'));
        }
