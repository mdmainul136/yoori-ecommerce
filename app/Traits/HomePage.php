...
use App\Http\Resources\SiteResource\MarqueeResource;
...
use App\Http\Resources\SiteResource\FlashSaleProductResource;
...
trait HomePage
{
    ...
    {
        return MarqueeResource::collection(\App\Models\Marquee::select('id', 'link')->where('status', 1)->get());
    }
            ...
            foreach ($settings as $key => $setting) {
                if (($key > $skip || $page == 1) && $key <= $content_count) {
                    foreach ($setting as $set_key => $item) {
                        if ($set_key == 'banner') {
                            ...
                            switch ($total):
                                case (4):
                                    ...
                                    $thumb = [
                                        @is_file_exists($image_0->image_variants['image_300x170'], $image_0->image_variants['storage']) ?
                                         @get_media($image_0->image_variants['image_300x170'], $image_0->image_variants['storage']) . '_0' :
                                         static_asset('images/default/default-image-650X301.png') . '_0' => $item['url'][0],


                                        @is_file_exists($image_1->image_variants['image_300x170'], $image_1->image_variants['storage']) ?
                                        @get_media($image_1->image_variants['image_300x170'], $image_1->image_variants['storage']) . '_1' :
                                        static_asset('images/default/default-image-650X301.png') . '_1' => $item['url'][1],

                                        @is_file_exists($image_2->image_variants['image_300x170'], $image_2->image_variants['storage']) ?
                                        @get_media($image_2->image_variants['image_300x170'], $image_2->image_variants['storage']) . '_2' :
                                        static_asset('images/default/default-image-650X301.png') . '_2' => $item['url'][2],

                                        @is_file_exists($image_3->image_variants['image_300x170'], $image_3->image_variants['storage']) ?
                                        @get_media($image_3->image_variants['image_300x170'], $image_3->image_variants['storage']) . '_3' :
                                        static_asset('images/default/default-image-650X301.png') . '_3' => $item['url'][3],
                                    ];
                                    $banners[] = $thumb;
                                    break;
                                case (3):
                                    ...
                                    $image_2 = $media->get($item['thumbnail'][2]);
                                    $thumb = [
                                        @is_file_exists($image_0->image_variants['image_650x301'], $image_0->image_variants['storage']) ?
                                        @get_media($image_0->image_variants['image_650x301'], $image_0->image_variants['storage']) . '_0' :
                                        static_asset('images/default/default-image-650X301.png') . '_0' => $item['url'][0],

                                        @is_file_exists($image_1->image_variants['image_650x301'], $image_1->image_variants['storage']) ?
                                         @get_media($image_1->image_variants['image_650x301'], $image_1->image_variants['storage']) . '_1' :
                                         static_asset('images/default/default-image-650X301.png') . '_1' => $item['url'][1],

                                        @is_file_exists($image_2->image_variants['image_650x301'], $image_2->image_variants['storage']) ?
                                        @get_media($image_2->image_variants['image_650x301'], $image_2->image_variants['storage']) . '_2' :
                                        static_asset('images/default/default-image-650X301.png') . '_2' => $item['url'][2],
                                    ];
                                    $banners[] = $thumb;
                                    break;
                        ...
                        // marquee section
                        if ($set_key == 'marquee_section') {
                            $component_names[] = $set_key;
                            $marquee_section = $this->getMarquee();
                            $results = $this->keyDefine('marquee_section', $key, $marquee_section, $results);
                            $keys[] = 'marquee_section';
                        }
                        ...
                        if ($set_key == 'popular_category') {
                            ...
                            // $popular_category = $category->categoryByIds($item);
                            $popular_category = TopCategoryResource::collection($category->categoryWithChild($item));
                        ...
                        if ($set_key == 'top_category') {
                            $component_names[] = $set_key;
                            $top_categories = TopCategoryResource::collection($category->categoryWithChild($item, ''));
                        ...
                        // all products....
                        if ($set_key == 'all_products') {
                            $component_names[] = $set_key;

                            if (in_array('all_products', $keys)) {
                                $position = array_search('all_products', array_values($keys));
                                $products_data = $results['all_products-' . $position];
                            } else {
                                $all_products = ProductResource::collection($this->product->allProduct());
                                ...
                                $products_data = [
                                    ...
                                    'best_selling_product' => $best_selling_products,
                                    'todays_deal' => $todays_deal,
                                    'latest_products' => $latest_products,
                                ];
                            }

                            $results = $this->keyDefine('all_products', $key, $products_data, $results);
                            $keys[] = 'all_products';
                        }
                        ...
                        if ($set_key == 'flash_deal') {
                            ...
                            if (in_array('flash_products', $keys)) {
                                ...
                                // $flash_products = $this->product->campaignProducts();
                                $flash_products = FlashSaleProductResource::collection($this->product->campaignProducts());
                            }
                        ...
                        if ($set_key == 'offer_ending_soon') {
                            $component_names[] = $set_key;
                            $image = @$media->get($item['banner']);
                            $image_2 = @$media->get($item['banner_2']);



                            $offer_end_sec_banner = $image && is_file_exists(@$image->image_variants['image_516x375'], @$image->image_variants['storage']) ? @get_media($image->image_variants['image_516x375'], $image->image_variants['storage']) : static_asset('images/default/510X369.png');
                            $offer_end_sec_banner_url = @$item['banner_url'];

                            $offer_end_sec_banner_2 = $image_2 && is_file_exists(@$image_2->image_variants['image_516x375'], @$image_2->image_variants['storage']) ? @get_media($image_2->image_variants['image_516x375'], $image_2->image_variants['storage']) : static_asset('images/default/510X369.png');
                            $offer_end_sec_banner_url_2 = @$item['banner_url_2'];


                            $offer_end = ProductResource::collection($this->product->offerEndingSoon(6));
                            ...
                            $results = $this->keyDefine('offer_ending_banner_2', $key, $offer_end_sec_banner_2, $results);
                            $results = $this->keyDefine('offer_ending_banner_url_2', $key, $offer_end_sec_banner_url_2, $results);
