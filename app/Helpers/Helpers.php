...
if (!function_exists('validate_purchase')):
    function validate_purchase($code, $data)
    {
        ...
        if (is_bool($response)) {
            $latest_version = isAppMode() ? '116' : '201';
        } else {
...
if (!function_exists('detail_special_price')) {
    ...
    {
        $lowest_price = supplierProductPriceCalculator($product->price, $product);
        $highest_price = supplierProductPriceCalculator($product->price, $product);

        if ($product->has_variant):
            foreach ($product->stock as $key => $stock):
                if ($lowest_price > $stock->price):
                    $lowest_price = supplierProductPriceCalculator($stock->price, $product);
                endif;
                if ($highest_price < $stock->price):
                    $highest_price = supplierProductPriceCalculator($stock->price, $product);
                endif;
            endforeach;
        endif;
        if (special_discount_applicable($product)):
...
if (!function_exists('getSvgLink')) {
    function getSvgLink($array, $storage = null)
    {
        $storage = $storage ?: (arrayCheck('storage', $array) ? $array['storage'] : null);
        $original = $array['original_image'] ?? null;

        if ($original && Str::endsWith(strtolower($original), '.svg')) {
            return is_file_exists($original, $storage)
                ? get_media($original, $storage)
                : static_asset('images/default/default-image.svg'); // fallback svg
        }

        // যদি SVG না হয়, fallback PNG দিয়ে দাও
        return static_asset('images/default/default-image-320x320.png');
    }
}
...
if (!function_exists('getImageWithSvgFallback')) {
    function getImageWithSvgFallback($size, $imageData, $default = null)
    {
        $storage = $imageData['storage'] ?? null;
        $original = $imageData['original_image'] ?? null;

        // If it's SVG, serve original image
        if ($original && Str::endsWith(strtolower($original), '.svg')) {
            return is_file_exists($original, $storage)
                ? get_media($original, $storage)
                : ($default ?? static_asset('images/default/default-image.svg'));
        }

        // Otherwise use getFileLink (e.g., image_72x72)
        return getFileLink($size, $imageData);
    }
}
    ...
    function getVideoId($provider, $link)
    {
        switch ($provider) {
            ...
            case 'youtube':
                ...
                if (preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|shorts/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $link, $match)) {
                    return $match[1] ?? '';
                }
                break;

            case 'embedded_video':
                if (preg_match('/src="([^"]+)"/', $link, $url)) {
                    return $url[1] ?? $link;
                }
                return $link;
