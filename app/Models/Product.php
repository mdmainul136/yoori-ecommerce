...
class Product extends Model
{
    ...
    public function getColorWiseVariantAttribute()
    {
        if (!empty($this->colors) && !empty($this->selected_variants)){
            return true;
        }
        else{
            return false;
    ...
    public function firstStock(): HasOne
    {
    ...
    public function userCompare(): HasOne
    {
    ...
    {
        //return @is_file_exists($this->thumbnail['image_190x230'] , $this->thumbnail['storage']) ? @get_media($this->thumbnail['image_190x230'],$this->thumbnail['storage']) : static_asset('images/default/210X205.png');

        return @is_file_exists($this->thumbnail['image_210x205'], $this->thumbnail['storage'])
            ? @get_media($this->thumbnail['image_210x205'], $this->thumbnail['storage'])
            : (@is_file_exists($this->thumbnail['image_190x230'], $this->thumbnail['storage'])
                ? @get_media($this->thumbnail['image_190x230'], $this->thumbnail['storage'])
                : static_asset('images/default/210X205.png'));
    }
    ...
    public function getDiscountPercentageAttribute()
    {
        ...
        $special_discount = $this->special_discount;
        $now = Carbon::now()->format('Y-m-d H:i:s');
        $price = supplierProductPriceCalculator($this->price, $this);

        if ($special_discount > 0 && $this->special_discount_start <= $now && $this->special_discount_end >= $now) {
            $type = $this->special_discount_type;

            if ($type == 'flat') {
                $amount = $price - $special_discount;
            } else {
                $amount = $price - ($price * ($special_discount / 100));
            }
        }
        return $amount;
    }
