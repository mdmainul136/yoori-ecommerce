...
class Category extends Model
{
    ...
    public function categoryLanguage(): HasMany
    {
        return $this->hasMany(CategoryLanguage::class);
    }
    public function ranges(): HasMany
    {
        return $this->hasMany(CategoryRange::class);
    }
    ...
    {
        if (@is_file_exists($this->logo['image_110x110'] ?? '', $this->logo['storage'] ?? '')) {
            return get_media($this->logo['image_110x110'], $this->logo['storage']);
        } elseif (@is_file_exists($this->logo['image_80x60'] ?? '', $this->logo['storage'] ?? '')) {
            return get_media($this->logo['image_80x60'], $this->logo['storage']);
        } else {
            return static_asset('images/default/132X132.png');
        }
    }
