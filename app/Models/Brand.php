...
class Brand extends Model
{
    ...
    {
        return $this->hasMany(BrandLanguage::class)->orderBy('title');
    }
    ...
    {
        return is_file_exists(@$this->logo['image_150x100'], @$this->logo['storage'])
        ? get_media(@$this->logo['image_150x100'], @$this->logo['storage'])
        : (is_file_exists(@$this->logo['image_130x93'], @$this->logo['storage'])
            ? get_media(@$this->logo['image_130x93'], @$this->logo['storage'])
            : static_asset('images/default/150X100.png'));
    }
